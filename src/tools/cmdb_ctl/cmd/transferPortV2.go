...
func transferProcToV3(s1, s3 dal.RDB, hostPortsMap map[string][]string) error {

	// 1、获取 这些ip的cc3.0的hostID
	hostIpIdMap, hostIdIpMap, hostIdPortsMap, err := getHostIpIdMap(s3, hostPortsMap)
	if err != nil {
		blog.Errorf("********************* get hostID failed, err: %v", err)
		return err
	}

	ids := make([]int64, 0)
	for _, id := range hostIpIdMap {
		ids = append(ids, id)
	}
	// 2、由于每天扫的ip数量比较少，根据hostID一次性全量获取1.0的进程信息即可。
	cond := mapstr.MapStr{
		common.BKHostIDField: mapstr.MapStr{
			common.BKDBIN: ids,
		},
	...
	if err := s1.Table(ccV1ProcessCollection).Find(cond).All(context.Background(), &v1Procs); err != nil {
		blog.Errorf(" get v1 procs failed, inputHostIDs: %v, err: %v", ids, err)
		return fmt.Errorf("find the result from db failed, %+v", err)
	}

	if len(v1Procs) == 0 {
		blog.Errorf("failed to get v1 process, inputHostIDs: %v", ids)
		return nil
	}
	// 对数据进行简单校验。获取主机和业务的关系和主机维度的进程关系
	procData := adjustProcData(v1Procs)

	hostIDModuleMap, hostIdBizMap, err := getV3HostModuleRelation(s3, ids)
	if err != nil {
		blog.Errorf("********************* get module relation failed, err: %v", err)
		return err
	}
	err = transProcDataV1ToV3Process(s3, procData, hostIDModuleMap, hostIdIpMap, hostIdPortsMap, hostIdBizMap)
	if err != nil {
...
// 按行读取文件，并整理成ip:[port1,port2] 格式
func readLine(fileName string) (map[string][]string, error) {
	...
	for {
		...
		if err != nil {
			...
			blog.Errorf("********************* read error line: %v, err: %v", line, err)
			return nil, err
		}
		lines := strings.Split(line, ":")
		if len(lines) != 2 {
			blog.Errorf("********************* ip format error ip: %v", line)
			continue
		}
		result[lines[0]] = append(result[lines[0]], lines[1])
	}
}
...
func runTransferPortCmd(conf *transferPortConf) error {
	...
	//解析文件，格式转化成ip:[port1,port2,port3]
	filename := "txt"
	hostIpPortsMap, err := readLine(filename)
	if err != nil {

		return err
	}

	if err := transferProcToV3(s1, s3, hostIpPortsMap); err != nil {
