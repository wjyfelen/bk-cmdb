...
func (s *Service) UpdateObjectAttribute(ctx *rest.Contexts) {
	...
	id, bizID, err := getAttrIDAndBizIDFromPath(ctx)
	if err != nil {
		ctx.RespAutoError(err)
		return
	}
...
func getAttrIDAndBizIDFromPath(ctx *rest.Contexts) (int64, int64, error) {
...
func (s *Service) getModelAttrByID(kit *rest.Kit, id, bizID int64) (metadata.Attribute, error) {
	...
	attrs, err := s.Engine.CoreAPI.CoreService().Model().ReadModelAttrByConditionForWeb(kit.Ctx, kit.Header,
		bizID, queryCond)
	if err != nil {
