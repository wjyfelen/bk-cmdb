...
type MapObjectSettings struct {
	// MaxDeep if the array element is the maximum level
	// allowed by the map object
	MaxDeep int

	// MaxLength the maximum number of elements per level
	// allowed by the array element if the map object.
	MaxLength int
}

// ValidateKVObject 对于通用的kv类型数据的校验。其中deep表示当前层级
